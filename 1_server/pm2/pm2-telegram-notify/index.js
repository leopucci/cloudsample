...
function sendTelegram(message) {
  ...
  if (!conf.telegram_url)
    return console.error(
      "There is no telegram URL set, please set the telegram URL: 'pm2 set pm2-telegram-notify:telegram_url https://telegram_url'"
    );
  ...
  if (!conf.log_err) {
    ...
    if (messages.length != 0 && event === 'log' && messages[0].event === 'error' && timestamp <= messages[0].timestamp) {
      ...
      if (messages[0].description.length > 30) {
        ...
        const length1 = 1000;
        const length2 = 3000;
        const cutDesc = description.substring(0, length1);
        const cutPrevDesc = messages[0].description.substring(0, length2);
